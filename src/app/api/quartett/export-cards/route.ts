...
function sanitizeFilename(value: string): string {
  const sanitized = value
    ...
    .replace(/^-+|-+$/g, "")
    .slice(0, 60);
  return sanitized || "quartett-cards";
}
...
export async function POST(req: NextRequest) {
  ...
  if (!block || block.type !== "quartett") {
    return NextResponse.json({ error: "Quartett block is required" }, { status: 400 });
  }
  ...
  const cards = buildQuartettCardVariants(block.items);
  if (cards.length === 0) {
  ...
  saveQuartettExportState(exportId, {
    title: body.worksheetTitle || block.title || "Quartett",
  ...
  try {
    ...
    const previewUrl = `${baseUrl}/${locale}/quartett/export-preview/${exportId}`;
    ...
    const cardElements = await page.$$('[data-quartett-export-card="true"]');
    ...
    for (let index = 0; index < cardElements.length; index += 1) {
      ...
      const filenameBase = sanitizeFilename(card.title || card.highlight || `${body.worksheetTitle || block.title || "quartett"}-${index + 1}`);
    ...
    const archiveName = sanitizeFilename(block.title || body.worksheetTitle || "quartett-cards");
    ...
    console.error("[QuartettExport] Failed to export cards:", error);
