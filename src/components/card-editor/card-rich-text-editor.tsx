...
export function CardRichTextEditor({
  ...
  const tc = useTranslations("cardEditor");
  const resolvedPlaceholder = placeholder ?? tc("textPlaceholder");
        ...
        <div className="flex flex-wrap items-center gap-0.5 border-b bg-muted/30 px-1 py-0.5">
          ...
          {TEXT_COLORS.map((c) => (
            <button
              ...
              title={
                c.value === "" ? tc("textColor_default") :
                c.value === "#FFFFFF" ? tc("textColor_white") :
                c.color
              }
              onMouseDown={(e) => {
