...
export function EBookDashboard() {
  ...
  const [searchResults, setSearchResults] = useState<EBookItem[] | null>(null);
  ...
  const [moveEbookId, setMoveEbookId] = useState<string | null>(null);
  ...
  const fetchContents = useCallback(async (folderId: string | null) => {
    ...
    try {
      ...
      const [foldersRes, ebooksRes] = await Promise.all([
        authFetch(`/api/folders?parentId=${folderId || ""}`),
        authFetch(`/api/ebooks?folderId=${folderParam}`),
      ]);
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const res = await authFetch(
          `/api/ebooks?search=${encodeURIComponent(search.trim())}`
        );
  ...
  const deleteFolder = async (folderId: string) => {
    if (!confirm(t("deleteChapter"))) return;
    try {
  ...
  const deleteEbook = async (ebookId: string) => {
    if (!confirm(t("deleteEbook"))) return;
    try {
      await authFetch(`/api/ebooks/${ebookId}`, { method: "DELETE" });
      if (searchResults) {
        setSearchResults(searchResults.filter((eb) => eb.id !== ebookId));
      }
      fetchContents(currentFolderId);
    } catch (err) {
  ...
  const moveEbookToFolder = async (targetFolderId: string | null) => {
    if (!moveEbookId) return;
    try {
      await authFetch(`/api/ebooks/${moveEbookId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
      ...
      console.error("Failed to move ebook:", err);
    }
  };

  const openMoveDialog = async (ebookId: string) => {
    setMoveEbookId(ebookId);
    try {
  ...
  const createNewEbook = async () => {
    try {
      const res = await authFetch("/api/ebooks", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          title: t("untitledEbook"),
          folderId: currentFolderId,
        }),
      });
      const data = await res.json();
      router.push(`/editor/ebook/${data.id}`);
    } catch (err) {
      console.error("Failed to create ebook:", err);
    }
  };

  const displayEbooks = searchResults !== null ? searchResults : ebooks;
  const isSearching = searchResults !== null;

  const countWorksheets = (chapters: EBookChapter[]) => {
    return chapters.reduce((acc, ch) => acc + ch.worksheetIds.length, 0);
  };
    ...
    <div className="px-6 py-10 overflow-y-auto flex-1">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button className="gap-2" onClick={createNewEbook}>
      ...
      {isSearching && (
        <p className="text-sm text-muted-foreground mb-4">
          {searchLoading
            ? tc("loading")
            : `${displayEbooks.length} result(s) for "${search}"`}
        </p>
      )}
      ...
      {loading && !isSearching ? (
        ...
        <>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">
                      ...
                      {folder._count.children > 0 &&
                        ...
                        " · "}
                      {folder._count.worksheets > 0 &&
                        `${folder._count.worksheets} items`}
                      {folder._count.children === 0 &&
                        folder._count.worksheets === 0 &&
                        tc("empty")}
                    </p>
          ...
          {/* E-Books */}
          {displayEbooks.length === 0 && folders.length === 0 ? (
            <Card className="border-dashed">
              <CardContent className="flex flex-col items-center justify-center py-16">
                <BookOpen className="h-12 w-12 text-muted-foreground/30 mb-4" />
                <h3 className="text-lg font-medium text-muted-foreground">
                  {isSearching ? t("noEbooks") : t("noEbooksYet")}
                </h3>
                <p className="text-sm text-muted-foreground/70 mt-1 mb-4">
                  {isSearching ? "" : t("createFirstEbook")}
                </p>
                {!isSearching && (
                  <Button onClick={createNewEbook}>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {displayEbooks.map((ebook) => (
                <div key={ebook.id} className="group relative">
                  <Link href={`/editor/ebook/${ebook.id}`}>
                    <Card className="hover:border-primary/50 hover:shadow-md transition-all cursor-pointer h-full">
                      <CardHeader className="pb-2">
                        ...
                        <CardDescription className="flex items-center gap-1 text-xs">
                          ...
                          {format.dateTime(new Date(ebook.updatedAt), {
                            day: "numeric",
                            month: "short",
                            year: "numeric",
                            hour: "2-digit",
                      ...
                      <CardContent>
                        <p className="text-xs text-muted-foreground">
                          {t("chapterCount", { count: ebook.chapters.length })} ·{" "}
                          {t("worksheetCount", {
                            count: countWorksheets(ebook.chapters),
                          })}
                        </p>
                      </CardContent>
                    </Card>
                  ...
                  <div className="absolute top-3 right-3 z-10">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem
                          onClick={() => router.push(`/editor/ebook/${ebook.id}`)}
                        >
                          <Pencil className="h-3.5 w-3.5 mr-2" />
                          {tc("edit")}
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => openMoveDialog(ebook.id)}>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button onClick={renameFolder} disabled={!renameFolderName.trim()}>
                {tc("rename")}
              </Button>
            </div>
          </div>
      ...
      {/* Move E-Book Dialog */}
      <Dialog
        open={moveEbookId !== null}
        onOpenChange={(open) => {
          if (!open) setMoveEbookId(null);
        }}
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
          ...
          <div className="space-y-2">
            <button
              ...
              onClick={() => moveEbookToFolder(null)}
            >
            ...
            {moveTargetFolders.map((folder) => (
              <button
                key={folder.id}
                className="flex items-center gap-2 w-full p-2 rounded hover:bg-muted transition-colors text-sm"
                onClick={() => moveEbookToFolder(folder.id)}
              >
