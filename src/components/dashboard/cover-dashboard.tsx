...
export function CoverDashboard() {
  const t = useTranslations("coverDashboard");
  ...
  const [searchResults, setSearchResults] = useState<CoverItem[] | null>(null);
  const [searchLoading, setSearchLoading] = useState(false);

  const fetchCovers = useCallback(async () => {
    setLoading(true);
    try {
      const res = await authFetch("/api/worksheets?type=covers");
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const res = await authFetch(
          `/api/worksheets?type=covers&search=${encodeURIComponent(search.trim())}`
        );
        const data = await res.json();
  ...
  const deleteCover = async (id: string) => {
    if (!confirm(t("deleteCover"))) return;
    try {
  ...
  const displayItems = searchResults !== null ? searchResults : covers;
    ...
    <div className="px-6 py-10 overflow-y-auto flex-1">
      ...
      {loading && !isSearching ? (
        ...
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center justify-center py-16">
            ...
            <h3 className="text-lg font-medium text-muted-foreground">
              {isSearching ? t("noCovers") : t("noCoversYet")}
            </h3>
            <p className="text-sm text-muted-foreground/70 mt-1 mb-4">
              {isSearching ? t("tryDifferentSearch") : t("createFirstCover")}
            </p>
            {!isSearching && (
              <Link href="/editor/covers">
                <Button>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {displayItems.map((cover) => (
            <div key={cover.id} className="group relative">
              <Link href={`/editor/covers/${cover.id}`}>
                <Card className="hover:border-primary/50 hover:shadow-md transition-all cursor-pointer h-full">
                  <CardHeader className="pb-2">
                    ...
                    <CardDescription className="flex items-center gap-1 text-xs">
                      ...
                      {format.dateTime(new Date(cover.updatedAt), {
                        day: "numeric",
                        month: "short",
                        year: "numeric",
                        hour: "2-digit",
                  ...
                  <CardContent>
                    <p className="text-xs text-muted-foreground">
                      {Array.isArray(cover.blocks)
                        ? t("elementCount", { count: cover.blocks.length })
                        : tc("empty")}
                    </p>
                  </CardContent>
                </Card>
              ...
              <div className="absolute top-3 right-3 z-10">
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    <DropdownMenuItem
                      onClick={() => router.push(`/editor/covers/${cover.id}`)}
                    >
