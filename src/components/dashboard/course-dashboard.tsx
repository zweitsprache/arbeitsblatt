...
import { CourseModule, countLessons, countTopics } from "@/types/course";
...
export function CourseDashboard() {
  ...
  const [searchResults, setSearchResults] = useState<CourseItem[] | null>(null);
  ...
  const fetchContents = useCallback(async (folderId: string | null) => {
    ...
    try {
      ...
      const [foldersRes, coursesRes] = await Promise.all([
        ...
        authFetch(`/api/courses?folderId=${folderParam}`),
      ]);
  ...
  const deleteCourse = async (courseId: string) => {
    if (!confirm(t("deleteCourse"))) return;
    try {
      await authFetch(`/api/courses/${courseId}`, { method: "DELETE" });
      if (searchResults) {
        setSearchResults(searchResults.filter((c) => c.id !== courseId));
      }
      fetchContents(currentFolderId);
    } catch (err) {
  ...
  const moveCourseToFolder = async (targetFolderId: string | null) => {
    if (!moveCourseId) return;
    try {
      await authFetch(`/api/courses/${moveCourseId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const openMoveDialog = async (courseId: string) => {
  ...
  const createNewCourse = async () => {
    try {
      ...
      const data = await res.json();
      router.push(`/editor/course/${data.id}`);
    } catch (err) {
    ...
    <div className="px-6 py-10 overflow-y-auto flex-1">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button className="gap-2" onClick={createNewCourse}>
      ...
      {isSearching && (
        <p className="text-sm text-muted-foreground mb-4">
          {searchLoading
            ? tc("loading")
            : `${displayCourses.length} result(s) for "${search}"`}
        </p>
      )}
      ...
      {loading && !isSearching ? (
        ...
        <>
          ...
          {/* Courses */}
          {displayCourses.length === 0 && folders.length === 0 ? (
            <Card className="border-dashed">
              <CardContent className="flex flex-col items-center justify-center py-16">
                <GraduationCap className="h-12 w-12 text-muted-foreground/30 mb-4" />
                <h3 className="text-lg font-medium text-muted-foreground">
                  {isSearching ? t("noCourses") : t("noCoursesYet")}
                </h3>
                <p className="text-sm text-muted-foreground/70 mt-1 mb-4">
                  {isSearching ? "" : t("createFirstCourse")}
                </p>
                {!isSearching && (
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {displayCourses.map((course) => (
                <div key={course.id} className="group relative">
                  <Link href={`/editor/course/${course.id}`}>
                    <Card className="hover:border-primary/50 hover:shadow-md transition-all cursor-pointer h-full">
                      <CardHeader className="pb-2">
                        ...
                        <CardDescription className="flex items-center gap-1 text-xs">
                          ...
                          {format.dateTime(new Date(course.updatedAt), {
                            day: "numeric",
                            month: "short",
                            year: "numeric",
                            hour: "2-digit",
                      ...
                      <CardContent>
                        <p className="text-xs text-muted-foreground">
                          {t("moduleCount", { count: course.structure.length })} ·{" "}
                          {t("topicCountSummary", {
                            count: countTopics(course.structure),
                          })} ·{" "}
                          {t("lessonCount", {
                            count: countLessons(course.structure),
                          })}
                        </p>
                      </CardContent>
                    </Card>
                  ...
                  <div className="absolute top-3 right-3 z-10">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem
                          onClick={() =>
                            router.push(`/editor/course/${course.id}`)
                          }
                        >
                          <Pencil className="h-3.5 w-3.5 mr-2" />
                          {tc("edit")}
                        ...
                        <DropdownMenuItem
                          onClick={() => openMoveDialog(course.id)}
                        >
      ...
      {/* Move Course Dialog */}
      <Dialog
        open={moveCourseId !== null}
        onOpenChange={(open) => {
          if (!open) setMoveCourseId(null);
        }}
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
          ...
          <div className="space-y-2">
            <button
              ...
              onClick={() => moveCourseToFolder(null)}
            >
            ...
            {moveTargetFolders.map((folder) => (
              <button
                key={folder.id}
                className="flex items-center gap-2 w-full p-2 rounded hover:bg-muted transition-colors text-sm"
                onClick={() => moveCourseToFolder(folder.id)}
              >
